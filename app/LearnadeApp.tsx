...
export default function LearnadeApp() {
  ...
  const saveMaterial = async (value:string,courseTitle:string,materialTitle:string,kind:MaterialKind,generated?:LearningPackage,courseId?:string,audio?:{blob:Blob;durationMs:number},aiGenerated=false) => {
    ...
    const material:CourseMaterial={id:materialId,title:cleanMaterialTitle,source:value,createdAt:now,kind,package:generated||generateLocalLearningPackage(value,cleanMaterialTitle),generation:{mode:aiGenerated&&generated?"on-device-ai":"local",generatedAt:now},audio:audio&&audioId?{id:audioId,mimeType:audio.blob.type||"audio/webm",size:audio.blob.size,durationMs:audio.durationMs}:undefined};
  ...
  const enhanceMaterial=async(courseId:string,materialId:string)=>{const course=library.find(item=>item.id===courseId);const material=course?.materials.find(item=>item.id===materialId);if(!course||!material)return;if(!confirm(`Enhance ${material.title} with the downloaded on-device AI? Its generated cards and questions will be replaced, and course study progress will reset.`))return;const generated=await generateWithBrowserAI(material.source,material.title,()=>{});const now=new Date().toISOString();const materials=course.materials.map(item=>item.id===materialId?{...item,package:generated,generation:{mode:"on-device-ai" as const,generatedAt:now}}:item);const updated={...course,materials,source:courseSource(materials),package:combineCourseMaterials(materials,course.title),updatedAt:now};["learnade-card-srs-","learnade-custom-cards-","learnade-cards-","learnade-quiz-","learnade-exam-","learnade-mastery-","learnade-diagnostic-"].forEach(prefix=>localStorage.removeItem(`${prefix}${courseId}`));await persistCourse(updated,false);if(activeId===courseId){setSource(updated.source);setTitle(updated.title);setLearningPackage(updated.package)}};  const deleteMaterial=async(courseId:string,materialId:string)=>{const course=library.find(item=>item.id===courseId);if(!course)return;if(course.materials.length===1){await deleteItem(courseId);setManageCourseId(null);return}if(!confirm("Remove this material from the course? Existing study progress for other materials will stay saved."))return;const material=course.materials.find(item=>item.id===materialId);const updated=removeCourseMaterial(course,materialId);if(material?.audio)await deleteLectureAudio(material.audio.id);localStorage.removeItem(`learnade-diagnostic-${courseId}`);await persistCourse(updated,false);if(activeId===courseId){setSource(updated.source);setTitle(updated.title);setLearningPackage(updated.package)}};
  ...
  return (
    <main className="app-shell" data-theme={theme}>
      ...
      {manageCourseId && library.find(item=>item.id===manageCourseId) && <ManageCourseModal course={library.find(item=>item.id===manageCourseId)!} onClose={()=>setManageCourseId(null)} onRename={updateCourseName} onDeleteMaterial={deleteMaterial} onAddMaterial={()=>{setManageCourseId(null);setUploadTarget({courseId:manageCourseId})}} onRecord={()=>{setManageCourseId(null);setRecordTarget({courseId:manageCourseId})}} onExam={()=>{const course=library.find(item=>item.id===manageCourseId);setManageCourseId(null);if(course)activateCourse(course,"exam")}} onRegenerate={()=>void regenerateCourse(manageCourseId)} onEnhance={materialId=>enhanceMaterial(manageCourseId,materialId)} />}
...
function UploadModal({ course, onClose, onCreate }: { course?:SavedCourse; onClose: () => void; onCreate: (text:string,courseTitle:string,materialTitle:string,kind:MaterialKind,generated?:LearningPackage,aiGenerated?:boolean) => Promise<void> }) {
  ...
  const create = async () => {
    ...
    try{await onCreate(text,course?.title||courseName,finalMaterialTitle,fileName?"upload":"pasted",generated,Boolean(useAI&&generated));}catch{setCreating(false);setError("Learnade could not save this material. Check available browser storage and try again.");}
  };
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="upload-title">
    <div className="modal">
      ...
      <label className="dropzone" onDragOver={event=>event.preventDefault()} onDrop={event=>{event.preventDefault();void selectFile(event.dataTransfer.files?.[0])}}>
...
function ManageCourseModal({course,onClose,onRename,onDeleteMaterial,onAddMaterial,onRecord,onExam,onRegenerate,onEnhance}:{course:SavedCourse;onClose:()=>void;onRename:(id:string,name:string)=>Promise<void>;onDeleteMaterial:(courseId:string,materialId:string)=>Promise<void>;onAddMaterial:()=>void;onRecord:()=>void;onExam:()=>void;onRegenerate:()=>void;onEnhance:(materialId:string)=>Promise<void>}) { const [name,setName]=useState(course.title);const [saving,setSaving]=useState(false);const [enhancing,setEnhancing]=useState<string|null>(null);const saveName=async()=>{setSaving(true);await onRename(course.id,name);setSaving(false)};const enhance=async(id:string)=>{setEnhancing(id);try{await onEnhance(id)}finally{setEnhancing(null)}};return <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="manage-title"><div className="modal manage-modal"><button className="modal-close" onClick={onClose} aria-label="Close">×</button><span className="eyebrow">COURSE SETTINGS</span><h2 id="manage-title">Manage {course.title}</h2><div className="rename-row"><label className="field"><span>Course name</span><input value={name} onChange={event=>setName(event.target.value)} /></label><button className="secondary" disabled={saving||name.trim()===course.title} onClick={saveName}>{saving?"Saving…":"Rename"}</button></div><div className="manager-actions"><button className="primary" onClick={onAddMaterial}>＋ Add material</button><button className="secondary" onClick={onRecord}>● Record lecture</button><button className="secondary" onClick={onExam}>✎ Make mock exam</button><button className="secondary" onClick={onRegenerate}>↻ Rebuild materials</button></div><div className="material-list"><span className="eyebrow">{course.materials.length} SAVED {course.materials.length===1?"ITEM":"ITEMS"}</span>{course.materials.map(material=><article key={material.id}><div className="material-kind" aria-hidden="true">{material.kind==="lecture"?"●":"▤"}</div><div><strong>{material.title}</strong><span>{material.kind} · {new Date(material.createdAt).toLocaleDateString()} · {material.source.split(/\s+/).length.toLocaleString()} words</span>{material.audio&&<AudioPlayer audioId={material.audio.id}/>}<small className="generation-status">{material.generation?.mode==="on-device-ai"?`On-device AI · ${new Date(material.generation.generatedAt).toLocaleDateString()}`:"Local study engine"}</small></div><button className="secondary material-enhance" disabled={enhancing===material.id} onClick={()=>void enhance(material.id)}>{enhancing===material.id?"Enhancing…":"Enhance with AI"}</button><button className="remove-material" onClick={()=>void onDeleteMaterial(course.id,material.id)} aria-label={`Remove ${material.title}`}>Remove</button></article>)}</div></div></div> }
...
function Cards({cards,sections,materials,learnadeId,onReview}:{cards:LearningPackage["flashcards"];sections:LearningPackage["sections"];materials:CourseMaterial[];learnadeId:string;onReview:(id:string)=>void}) {
  const storageKey=`learnade-card-srs-${learnadeId}`;const [selectedMaterials,setSelectedMaterials]=useState<string[]>(()=>materials.map(material=>material.id));const customKey=`learnade-custom-cards-${learnadeId}`;const [customCards,setCustomCards]=useState<LearningPackage["flashcards"]>(()=>{try{const saved=JSON.parse(localStorage.getItem(customKey)||"[]");return Array.isArray(saved)?saved:[]}catch{return[]}});const [adding,setAdding]=useState(false);const [front,setFront]=useState("");const [back,setBack]=useState("");useEffect(()=>{localStorage.setItem(customKey,JSON.stringify(customCards))},[customCards,customKey]);
  const allCards=useMemo(()=>[...cards,...customCards].filter(card=>card.id.startsWith("custom-")||selectedMaterials.some(id=>card.id.startsWith(`${id}:`))),[cards,customCards,selectedMaterials]);const allIds=useMemo(()=>allCards.map(card=>card.id),[allCards]);const [queue,setQueue]=useState<string[]>(allIds);const [reviews,setReviews]=useState<Record<string,CardReview>>({});const [flip,setFlip]=useState(false);const [status,setStatus]=useState("");const [hydrated,setHydrated]=useState(false);
...
function Quiz({questions,sections,learnadeId,onReview}:{questions:LearningPackage["quiz"];sections:LearningPackage["sections"];learnadeId:string;onReview:(id:string)=>void}) {
  ...
  if(complete)return <div className="quiz-panel quiz-results"><span className="completion-mark">{score>=Math.ceil(questions.length*.8)?"✓":"↻"}</span><span className="eyebrow">QUIZ COMPLETE</span><h2>{score>=Math.ceil(questions.length*.8)?"Excellent work.":"Quiz complete."} {score} out of {questions.length}</h2><p>{score>=Math.ceil(questions.length*.8)?"You have a solid grasp of this material. Keep that momentum going.":"Good start. Another pass will strengthen the concepts that are still forming."}</p><button className="primary" onClick={restart}>Try the quiz again</button></div>;
...
function MockExam({courseTitle,materials,learnadeId,onReview}:{courseTitle:string;materials:CourseMaterial[];learnadeId:string;onReview:(id:string)=>void}) {
  ...
  const [selectedIds,setSelectedIds]=useState<string[]>(allIds);const [requested,setRequested]=useState(10);const [timeLimit,setTimeLimit]=useState(0);const [phase,setPhase]=useState<"setup"|"taking"|"results">("setup");const [activeExam,setActiveExam]=useState<MockExamData|null>(null);const [answers,setAnswers]=useState<Array<number|null>>([]);const [index,setIndex]=useState(0);const [edition,setEdition]=useState(1);const [remaining,setRemaining]=useState<number|null>(null);const recordedRef=useRef(false);
  ...
  const start=()=>{let previous:string[]=[];try{previous=JSON.parse(localStorage.getItem(`learnade-exam-${learnadeId}`)||"null")?.questionIds||[]}catch{}const exam=buildMockExam(materials,selectedIds,effectiveCount,courseTitle,previous,edition);if(!exam.questions.length)return;recordedRef.current=false;setActiveExam(exam);setAnswers(exam.questions.map(()=>null));setIndex(0);setRemaining(timeLimit?timeLimit*60:null);setPhase("taking")};
